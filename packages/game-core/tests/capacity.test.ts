...
describe('capacityOf', () => {
  it('returns capacity for grade 1', () => {
    expect(capacityOf(1)).toBe(1_000n)
  })

  it('returns capacity for grade 10', () => {
    expect(capacityOf(10)).toBe(100_000_000n)
  })
...
describe('computeFree', () => {
  it('returns remaining capacity', () => {
    expect(computeFree(1, { GRAIN: 300n })).toBe(700n)
  })

  it('clamps to 0n when over capacity', () => {
    expect(computeFree(1, { GRAIN: 2_000n })).toBe(0n)
  })
})

describe('isFull', () => {
  it('is true when used equals capacity', () => {
    expect(isFull(1, { GRAIN: 1_000n })).toBe(true)
  })

  it('is false when below capacity', () => {
    expect(isFull(1, { GRAIN: 999n })).toBe(false)
  })
})
