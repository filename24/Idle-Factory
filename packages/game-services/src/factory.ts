...
import { runInTx, ServiceError, Tx } from './base'
...
function applyXpBonus(baseXp: bigint, bonusBps: number): bigint {
  return (baseXp * (XP_BONUS_DENOMINATOR_BPS + BigInt(bonusBps))) / XP_BONUS_DENOMINATOR_BPS
}
...
export const FactoryService = {
  async build(prisma: PrismaClient, params: BuildParams) {
    const { userId, type, anchorX, anchorY, landIndex = DEFAULT_LAND_INDEX, guildId } = params
    ...
    return runInTx(prisma, async (tx) => {
      ...
      const buildXp = applyXpBonus(xpForEvent({ kind: 'BUILD', cost }), resolveXpBonusBps(credit))
      ...
      const cells = getOccupiedCells(anchorX, anchorY, entry.size.width, entry.size.height)
   ...
   */
  async destroy(prisma: PrismaClient, params: DestroyParams): Promise<DestroyResult> {
    const { userId, factoryId } = params
    return runInTx(prisma, async (tx) => {
      const factory = await tx.factory.findUnique({ where: { id: factoryId } })
      if (!factory || factory.userId !== userId) throw new ServiceError('FACTORY_NOT_FOUND')
  ...
  > {
    ...
    const filtered = q
      ? rows.filter((f) => f.id.toLowerCase().includes(q) || f.type.toLowerCase().includes(q))
      : rows
