...
import { runInTx, ServiceError, type ServiceErrorCode, type Tx } from './base'
...
export const GuildService = {
   ...
   * 표시하지 않는다.
   */
  async upsertOnJoin(prisma: PrismaClient, input: UpsertOnJoinInput): Promise<GuildRow> {
  ...
  /** 어드민 액션 — 길드 언어 변경. */
  async updateLang(prisma: PrismaClient, guildId: string, lang: string): Promise<GuildRow> {
    const exists = await prisma.guild.findUnique({
      where: { id: guildId },
      select: { id: true },
    })
    if (!exists) {
      throw new ServiceError('USER_NOT_FOUND', `guild ${guildId} not registered`)
    }
   ...
   */
  async getCreditEffects(prisma: PrismaLike, guildId: string): Promise<CreditEffects> {
  ...
  async assertCreditAtLeast(
    ...
    const credit = guild?.credit ?? CREDIT_DEFAULT
    if (credit < min) {
      throw new ServiceError(errorCode, `guild ${guildId} credit ${credit} < ${min}`, {
        credit,
        min,
      })
    }
  ...
  async applyWeeklyCredit(prisma: PrismaClient): Promise<WeeklyCreditResult> {
    ...
    for (const g of guilds) {
      const nextCredit = applyCreditDelta(g.credit, weeklyCreditDelta(g.weeklyDAU))
      // 스냅샷 기준 긴급 대상 여부 — 실제 보전은 tx 내 재조회로 확정.
      const maybeEmergency =
        nextCredit < EMERGENCY_CREDIT_THRESHOLD && g.vault < EMERGENCY_SUPPORT_CAP
      ...
      const emergencyApplied = await runInTx(prisma, async (tx) => {
        ...
        await tx.guild.update({
          ...
          data: {
            credit: nextCredit,
            ...(needsTopUp ? { vault: { increment: EMERGENCY_SUPPORT_CAP - fresh.vault } } : {}),
          },
        })
        return needsTopUp
      })
   ...
   * @returns 복원된 금고액(복원 없으면 0n).
   */
  async restoreInactivePool(prisma: PrismaClient, guildId: string): Promise<bigint> {
