...
async function buildSynergyMapsForFactories(
  ...
  for (const land of lands) {
    ...
    result.set(land.id, computeLandSynergies({ factories: factoriesInput, slots: slotsInput }))
  }
  ...
  return runInTx(prisma, async (tx) => {
    ...
    for (const factory of factories) {
      ...
      for (const slot of factory.slots) {
        ...
        const b = getSpecialSlotBonus(slot.type as SlotType, factory.type as FactoryState['type'])
        if (b > slotBonus) slotBonus = b
      }

      const synergyMap = synergyByLand.get(factory.landId)
      const synergyBonus = synergyMap ? getSynergyMultiplier(synergyMap, factory.id) : 1.0
      ...
      const state: FactoryState = {
        ...
        upgradeBooster: factory.upgradeBooster as FactoryState['upgradeBooster'],
      ...
      for (const [mat, amt] of Object.entries(result.consumed) as Array<[MaterialType, bigint]>) {
        if (!amt || amt <= 0n) continue
        await applyMaterialDelta(tx, warehouse.id, mat, -amt)
        balance[mat] = (balance[mat] ?? 0n) - amt
      }

      for (const [mat, amt] of Object.entries(result.produced) as Array<[MaterialType, bigint]>) {
        if (!amt || amt <= 0n) continue
        await applyMaterialDelta(tx, warehouse.id, mat, amt)
        balance[mat] = (balance[mat] ?? 0n) + amt
      }
      ...
      if (rareDrop > 0n) {
        ...
        // 헬퍼를 거친다.
        const roomForBooster = unitsThatFit('RAW_BOOSTER', computeFree(warehouse.grade, balance))
        ...
        if (credited > 0n) {
          ...
          producedForSummary.RAW_BOOSTER = (producedForSummary.RAW_BOOSTER ?? 0n) + credited
        }
      }

      const advancedAt = new Date(factory.lastHarvestAt.getTime() + result.ticksRealized * TICK_MS)
    ...
    if (totalTicks > 0) {
      ...
      const applied = applyXp({ level: user.level, xpInLevel: user.xp }, xpGained)
...
async function accrueStockProfits(
  ...
  const listedSummaries = summaries.filter((s) => stockByFactory.has(s.factoryId))
  const materials = [
    ...new Set(listedSummaries.flatMap((s) => Object.keys(s.produced) as MaterialType[])),
  ]
  ...
  for (const summary of listedSummaries) {
    ...
    let amount = 0n
    for (const [mat, qty] of Object.entries(summary.produced) as Array<[MaterialType, bigint]>) {
