...
export async function generateMetadata(): Promise<Metadata> {
  const t = await getTranslations('authError.meta')
  return {
    title: t('title'),
    ...
    >
      <div className="border-hairline-strong bg-surface w-full max-w-md rounded border p-8">
        <div className="space-y-4 text-center">
          ...
          >
            {t(`${resolved.messageCode}.title`)}
          </h1>
          <p className="text-charcoal text-sm leading-relaxed break-keep">
            {t(`${resolved.messageCode}.description`)}
          </p>
        </div>
        ...
        <dl className="border-hairline mt-8 border-t pt-5 text-xs">
          <div className="flex items-baseline justify-between gap-4">
            <dt className="text-mute shrink-0">{t('detail.code')}</dt>
            <dd className="text-charcoal truncate font-mono">{resolved.code}</dd>
          </div>
          {errorDescription && (
            <div className="mt-2 flex items-baseline justify-between gap-4">
              <dt className="text-mute shrink-0">{t('detail.detail')}</dt>
              <dd className="text-charcoal break-all">{errorDescription}</dd>
            </div>
          )}
          {!resolved.known && (
            <p className="text-mute mt-3 leading-relaxed break-keep">{t('detail.unknownHint')}</p>
          )}
        </dl>
      </div>
    </section>
