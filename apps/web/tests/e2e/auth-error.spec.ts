...
import { UNKNOWN_AUTH_ERROR_CODE } from '../../src/lib/auth-errors'

const heading = '#auth-error-heading'

/**
 * 기대 제목은 한국어 메시지 카탈로그에서 읽는다.
 * e2e 는 쿠키를 세팅하지 않으므로 기본 로케일(ko)로 렌더된다.
 */
const title = (code: string): string =>
  (ko.authError as unknown as Record<string, { title: string }>)[code]!.title

test.describe('인증 실패 안내 페이지', () => {
  test('error 파라미터 없이 접근해도 폴백 안내를 렌더한다', async ({ page }) => {
    const response = await page.goto('/auth/error')
    expect(response?.status()).toBe(200)
    await expect(page.locator(heading)).toHaveText(title(UNKNOWN_AUTH_ERROR_CODE))
  })

  test('사용자 취소(access_denied)를 재시도 가능한 안내로 보여준다', async ({ page }) => {
    await page.goto('/auth/error?error=access_denied')
    await expect(page.locator(heading)).toHaveText(title('access_denied'))
  ...
  test('설정 오류(invalid_client)에는 재시도 대신 문의를 안내한다', async ({ page }) => {
    ...
    await expect(page.locator(heading)).toHaveText(title('invalid_client'))
