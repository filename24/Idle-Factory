...
- 기본 3×3 토지에 **창고 1개 필수** → 공장 배치 가능 슬롯은 8개 (설계 목표)
- 창고를 철거할 수는 없음 (필수 건물)
- 창고도 이모지로 표시: 📦 (등급은 상첨자: 📦³)

> **미구현**: 위 슬롯 점유 규칙은 확정 스펙(U-7, 2026-07-03)이나 현재 코드에는 반영되지
> 않았다. `Warehouse` 모델(`packages/database/prisma/schema.prisma`)에 슬롯 좌표
> 필드가 없고, `canPlace()`(`packages/game-core/src/land/layout.ts`)도 창고 점유를
> 검사하지 않는다. 그 결과 기본 3×3 토지는 현재 9칸 전부 공장 배치가 가능하다(8개가
> 아니다). 창고의 슬롯 점유가 구현되면 배치 가능 슬롯은 8개로 줄어든다. 상세는
> `11-land.md` §특수 슬롯 생성 규칙(U-7 결정) 참고.

### 예시 (3×3 기본 토지, 구현 후 목표 모습)
