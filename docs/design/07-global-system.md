...
> **보류 (모순 9)**: 서버(로컬) 주식 참여에 **신뢰도 하한**(예: 500+)을 둘지는 미확정이다. 현재 코드·스키마는 서버 주식을 **Lv.10** 로만 게이팅한다(08-stock.md 참조). 신뢰도 구간표에 서버 주식 밴드를 정식 편입할지는 **Phase 4 전** 결정한다.

### 신뢰도 변동 — 활성도 기반

**주 1회 정산 시 (일요일 00:00 KST)** 해당 서버의 DAU 활성도로 신뢰도를 조정합니다.
...
- **창고 자재**: `GlobalMarketPrice.currentPrice` 기준의 미실현 평가액.
- **공장 평가액 = 누적 투자비**(v1). 시세 재평가 대신 투입 원가로 산정해 계산을 단순화한다.
- v1 스코프 — 세금 정산·상장 자격·IPO 기본가 산정에 공통 적용.

### 정산 주기

- **주 1회 정산** — 매주 **일요일 00:00 KST** 기준 (cron 은 UTC 토요일 15:00 로 등록) (U-6, 2026-07-03 확정)
...
> **보류 (U-8)**: 유저가 게임을 **탈퇴**했다가 **재가입**할 때 자산을 초기화할지(또는 복원할지)는 미확정이다. 탈퇴 개념 도입 여부와 함께 **Phase 3 전** 결정한다. (00-onboarding.md §미결정 사항과 동일 항목)
...
- `Guild.credit` — 서버 신뢰도 (0 ~ 2000, 기본 1000 · 코드 현행)
