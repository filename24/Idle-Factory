...
- After modifying code, run `graphify update .` to keep the graph current (AST-only, no API cost). The `post-commit` and `post-merge` hooks do this automatically in the background — `post-commit` covers local commits, `post-merge` covers `git pull` / merges (without it, PRs merged on GitHub and pulled down leave the graph silently stale).
